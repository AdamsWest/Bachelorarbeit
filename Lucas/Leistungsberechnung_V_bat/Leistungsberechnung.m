...
i_bat = 29.7754;                  % i
R_bat = 0.0142;               % R_bat
Batterie_data = [Q Qnom Qexp Vfull Vexp Vnom i_bat R_bat 0 0 0];
[Eo,A,K] = Batterie_parameter(Batterie_data);
Batterie_data = [Q Qnom Qexp Vfull Vexp Vnom i_bat R_bat Eo A K];       % vollständiger Vektor
...
for h = H_0:Delta_H:H_max
    ...
    if Thrust(i) > max(max(T_map))                                  % wenn Schub zu gross (Ergebnis verwerfen)
        ...
        if i == 1
            [I_Bat(i),U_Bat(i),C_rate,Delta_C_Bat,C_Rest_V(i),i_int(1)] = Batterie(PWM(i),eta_PWM,I_mot(i),...
                n_Prop,C_Bat,P_Bat,Delta_C_Bat,t_Flug,0,N_Bat_cell,Batterie_data);
        else
            
            [I_Bat(i),U_Bat(i),C_rate,Delta_C_Bat,C_Rest_V(i),i_int(i)] = Batterie(PWM(i),eta_PWM,I_mot(i),...
                n_Prop,C_Bat,P_Bat,Delta_C_Bat,t_Flug,i_int(i-1),N_Bat_cell,Batterie_data);
        end
