...
V_Kg_g = zeros(lengthi,1);
t_Flug_g = zeros(lengthi+1,1);
% Propeller
Thrust = zeros(lengthi,1);
Omega_g = zeros(lengthi,1);
tau_g = zeros(lengthi,1);
...
% Motor
U_mot_g = zeros(lengthi,1);
I_mot_g = zeros(lengthi,1);
eta_mot = zeros(lengthi,1);
% ESC
PWM_g = zeros(lengthi,1);
eta_PWM = zeros(lengthi,1);
% Batterie
C_Rate = zeros(lengthi,1);
C_Rest_V_g = zeros(lengthi,1);
I_Bat_g = zeros(lengthi,1);
U_Bat_g = zeros(lengthi+1,1);
U_Bat_g(1) = U_Bat_nom;                               % Startspannung ist die Nominalspannung
P_Bat = zeros(lengthi,1);
Delta_C_Bat = zeros(lengthi+1,1);
i_int = zeros(lengthi+1,1);
% Gesamtsystem
eta_ges_g = zeros(lengthi,1);
...
%                 eta_prop_ueber(w) = (Thrust_ueber(w) * (mu_z_ueber(w) + vi_ueber(w)))/(tau_ueber(w) * Omega_ueber(w));
                ...
                [PWM_ueber(w),eta_PWM_ueber(w)] = ESC(U_mot_ueber(w),U_Bat_g(x));         % <-- hier U_bat_inter
            ...
            if C_Rest_V_ueber(w) < 0.0 || U_mot_ueber(w) > U_Bat_g(x) || U_mot_ueber(w) <= 0 || C_Rate_ueber(w) > C_Rate_max || I_mot_ueber(w) > I_max || ...
            ...
            if x > 1     % Entferne alle Ergebnisse, wobei die Restladung im Vergleich zum vorheigen Schritt steigt
                if C_Rest_V_ueber(w) > C_Rest_V_g(x-1)
                    C_Rest_V_ueber(w) = NaN;
                    Omega_ueber(w) = NaN;
                    U_mot_ueber(w) = NaN;
                    I_mot_ueber(w) = NaN;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if isnan(mean(good_omega)) ~= 1 && isnan(mean(good_I_mot)) ~= 1 &&  isnan(mean(good_U_mot)) ~= 1 && ...     % Wenn alle der Vektoren nicht nur NaN
        ...
        V_Kg_g(x) = V_Kg_inter(ind_opt2);	    % Bestimmung opt. Stgeschw. und speichern in Vektor für jeden Höhenabschnitt
        t_Flug_g(x+1) = t_Flug_g(x) + t_Flug_inter(ind_opt2);
        % Propeller
        Thrust(x) = Thrust_inter(ind_opt2);
        Omega_g(x) = Omega_inter(ind_opt2);
        tau_g(x) = tau_inter(ind_opt2);
        ...
        % Motor
        I_mot_g(x) = I_mot_inter(ind_opt2);
        U_mot_g(x) = U_mot_inter(ind_opt2);
        eta_mot(x) = eta_mot_inter(ind_opt2);
        % ESC
        PWM_g(x) = PWM_inter(ind_opt2);
        eta_PWM(x) = eta_PWM_inter(ind_opt2);
        % Batterie
        I_Bat_g(x) = I_Bat_inter(ind_opt2);
        U_Bat_g(x+1) = U_Bat_inter(ind_opt2);
        C_Rate(x) = C_Rate_inter(ind_opt2);
        C_Rest_V_g(x) = C_Rest_V_inter(ind_opt2);
        Delta_C_Bat(x+1) = Delta_C_Bat_inter(ind_opt2);
        i_int(x+1) = i_int_inter(ind_opt2);
        % Gesamtsystem
        eta_ges_g(x) = eta_ges_inter(ind_opt2);
...
H2 = [0;H];
subplot(626), stairs(H2,U_Bat_g,'LineWidth',1), grid, title('Batteriespannung'), xlabel('Höhe [m]'),ylabel('U_{Bat} [V]')
subplot(627), stairs(H,PWM_g*100,'LineWidth',1), grid, title('Pulsweitenmodulation'), xlabel('Höhe [m]'),ylabel('PWM [%]')
subplot(628), stairs(H,eta_ges_g*100,'LineWidth',1)
hold on
...
hold off
subplot(629), stairs(H,V_Kg_g,'LineWidth',1), title('Bahngeschwindigkeit'), grid, xlabel('Höhe [m]'),ylabel('V_{Kg} [m/s]')
subplot(6,2,10), stairs(H2,t_Flug_g,'LineWidth',1), title('Flugzeit'), grid, xlabel('Höhe [m]'),ylabel('t_{Flug} [s]')
...
subplot(323), stairs(H,Omega_g/(2*pi)*60,'LineWidth',1), grid, title('Drehzahl'), xlabel('Höhe [m]'),ylabel('Drehzahl [RPM]')
    hold on 
    stairs(H,Omega_g/(2*pi)*60./Uebersetzung,'LineWidth',1)
    legend('Motordrehzahl', 'Propellerdrehzahl','Location','southeast')
subplot(324), stairs(H,tau_g,'LineWidth',1), grid, title('Drehmoment'), xlabel('Höhe [m]'),ylabel('Drehmoment [Nm]')
    hold on 
    stairs(H,tau_g .* Uebersetzung,'LineWidth',1), legend('Motordrehmoment','Propellerdrehmoment','Location','northeast');
subplot(325), stairs(H,Omega_g.*tau_g,'LineWidth',1), grid, title('Gegenüberstellung der Leistungen'), xlabel('Höhe [m]'), ylabel('Leistung [W]');
    hold on 
