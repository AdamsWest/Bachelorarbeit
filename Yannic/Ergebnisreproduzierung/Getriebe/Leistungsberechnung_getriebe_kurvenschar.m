...
%             m = m_copter + m_Bat + (m_Mot + m_getriebe) * n_Prop + m_nutz;                     % Gesamtmasse des Quadrocopters
    ...
    subplot(621), l1(j) = plot(H,C_Rest_V*100,'LineWidth',2); l1_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(622), l2(j) = plot(H,Omega/(2*pi)*60,'LineWidth',2); l2_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(623), l3(j) = plot(H,I_mot,'LineWidth',2); l3_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(624), l4(j) = plot(H,U_mot,'LineWidth',2); l4_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(625), l5(j) = plot(H,I_Bat,'LineWidth',2); l5_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    H2 = [0;H];
    subplot(626), l6(j) = plot(H2,U_Bat,'LineWidth',2); l6_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(627), l7(j) = plot(H,PWM*100,'LineWidth',2); l7_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(628), l8(j) = plot(H,eta_ges*100,'LineWidth',2); l8_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(629), l9(j) = plot(H,V_Kg,'LineWidth',2); l9_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(6,2,10), l10(j) = plot(H2,t_Flug,'LineWidth',2); l10_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    subplot(6,2,11), l11(j) = plot(H,Uebersetzung,'LineWidth',2); l11_Info{j} = ['m_{Getriebe} = ' num2str(m_getriebe) ' kg']; grid on, hold on
    %% Spielereien
...
subplot(621), title('Restladung'), xlabel('Höhe [m]'),
ylabel('C_{Bat,Rest} [%]'), legend([l1], l1_Info)
subplot(622), title('Drehzahl'), xlabel('Höhe [m]'),
ylabel('Drehzahl [RPM]'), legend([l2], l2_Info)
subplot(623), title('Motorstrom'), xlabel('Höhe [m]'),
ylabel('I_{Mot} [A]'), legend([l3], l3_Info)
subplot(624), title('Motorspannung'), xlabel('Höhe [m]'),
ylabel('U_{mot} [V]'), legend([l4], l4_Info)
subplot(625), title('Batteriestrom'), xlabel('Höhe [m]'),
ylabel('I_{Bat} [%]'), legend([l5], l5_Info)
H2 = [0;H];
subplot(626), title('Batteriespannung'), xlabel('Höhe [m]'),
ylabel('U_{Bat} [A]'), legend([l6], l6_Info)
subplot(627), title('Pulsweitenmodulation'), xlabel('Höhe [m]'),
ylabel('PWM [%]'), legend([l7], l7_Info)
subplot(628), title('Gesamtwirkungsgrad'), xlabel('Höhe [m]'),
ylabel('eta_{ges} [%]'), legend([l8], l8_Info)
subplot(629), title('Bahngeschwindigkeit'), xlabel('Höhe [m]'),
ylabel('V_{Kg} [m/s]'), legend([l9], l9_Info)
subplot(6,2,10), title('Flugzeit'), xlabel('Höhe [m]'),
ylabel('t_{Flug} [s]'), legend([l10], l10_Info)
subplot(6,2,11), title('Uebersetzung'), xlabel('Höhe [m]'),
ylabel('i'), legen([l11], l11_Info)
