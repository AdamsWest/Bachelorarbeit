...
Dateiname = 'aeromet_nutzlast_redu';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% *************************************************************************
Abfrage_mot = [21 21 21];
Abfrage_prop = {'10x4','10x4','10x4'};
Abfrage_n_prop = [1 2 4 6 8];
Abfrage_c_W = [0.1 0.5 1 1.5 2];
Abfrage_n_bat = [5 5 5];
Abfrage_m_nutz = [0.25 0 0.25];
Abfrage_uwg = [10 100/3.6 100/3.6];
...
for j = 1:length(Abfrage_mot)
    ...
    m_Bat = m_ges * 2/3;              % Batteriemasse
    m_copter = m_ges * (0.354/1.06);           % Leermasse
    
    
    % Batterie
    E_Dichte = 890540;      % Energiedichte des LiPos in J/kg
    N_Bat_cell = Abfrage_n_bat(j);         % Anzahl der Batteriezellen in Reihe
    ...
    % Missionsparameter
    m_nutz = Abfrage_m_nutz(j);          % Nutzlast in kg
%     m_Bat = m_Bat - m_nutz;
    ...
    c_W_copter_oben = Abfrage_c_W(2);                    % Widerstandsbeiwert des Multicopters
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    l9_Info{j} = ['m_{Nutz} = ' num2str(m_nutz) ' kg, u_{Wg} = ' num2str(u_Wg) ' m/s']; grid on, hold on
...
ylabel('V_{Kg} [m/s]'),
lgd = legend([l9], l9_Info, 'Location','bestoutside'); title(lgd,'AEROMET\_UAV Randbedingungen')  
% 
